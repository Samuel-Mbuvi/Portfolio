...
function Certification({ title, organization, earnedOn, link }) {
  return (
    <article className="pt-8 border-b-2 border-dark-content pb-5 dark:border-light-content border-opacity-20 dark:border-opacity-20">
      <div className="flex justify-between items-center">
        ...
        {/* Conditionally show the button if link exists */}
        {link && (
            <a
            href={link}
            target="_blank"
            rel="noopener noreferrer"
          className="btn bg-greenbg text-green-text text-xs inline-block rounded-3xl px-3 py-1 min-w-fit hover:underline"
        >
          View Certificate
        </a>
        )} 
      ...
      <div className="flex justify-between pt-2">
        <div className="flex items-center">
          ...
          <p className="text-content text-xs md:text-sm font-light pl-1">{organization}</p>
        </div>
